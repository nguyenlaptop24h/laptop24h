...
function addScrollBtns(){
  ...
  t.style.cssText=base+';bottom:80px';
  ...
  window.addEventListener('scroll',function(){
    ...
    var atBot=(window.innerHeight+window.scrollY)>=document.body.scrollHeight-50;
    t.style.display=atTop?'none':'block';
    b.style.display=atBot?'none':'block';
  },{passive:true});
...
if(!tryActivate()){
  ...
  var iv=setInterval(function(){
    attempts++;
    if(tryActivate()||attempts>=120)clearInterval(iv);
  },500);
}
})();